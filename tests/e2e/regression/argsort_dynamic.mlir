...
// result is unused, but sort still requires writable key storage. Returning the
// input also verifies that copy-on-write preserves its tensor value.
// CHECK-LABEL: EXEC @argsort

#identity = affine_map<(d0) -> (d0)>

func.func @argsort(%input: tensor<?xi64>)
  ...
  return %sorted#1, %input : tensor<?xi64>, tensor<?xi64>
}
